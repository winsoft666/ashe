...
HRESULT Registry::open(REGSAM samDesired, bool bCreate) noexcept {
    LSTATUS dwResult;
    close();
...
HRESULT Registry::waitForChange(DWORD dwChangeFilter, bool bWatchSubtree) noexcept {
    LSTATUS ls = RegNotifyChangeKeyValue(m_hkey, bWatchSubtree, dwChangeFilter, NULL, FALSE);
    return HRESULT_FROM_WIN32(ls);
}

bool Registry::DeleteKey(HKEY hKey, LPCWSTR pszSubKey, LPCWSTR pszValName, bool bPrefer64View) noexcept {
    HKEY hSubKey = NULL;
...
bool Registry::DeleteSubKeys(HKEY hKeyRoot, LPCTSTR lpSubKey, bool bPrefer64View) noexcept {
...
HRESULT Registry::getDWORDValue(LPCWSTR pszValueName, OUT DWORD& pdwDataOut) const noexcept {
    return getValue(pszValueName, REG_DWORD, (LPBYTE)(&pdwDataOut), sizeof(DWORD));
}

HRESULT Registry::getBINARYValue(LPCWSTR pszValueName, LPBYTE pbDataOut, int cbDataOut) const noexcept {
    return getValue(pszValueName, REG_BINARY, pbDataOut, cbDataOut);
}

HRESULT Registry::getSZValue(LPCWSTR pszValueName, OUT std::wstring& strValue) const noexcept {
    DWORD cb = 0;
...
HRESULT Registry::getExpandSZValue(LPCWSTR pszValueName,
...
HRESULT Registry::getMultiSZValue(LPCWSTR pszValueName, OUT std::vector<std::wstring>& vStrValues) const noexcept {
    DWORD cb = 0;
    HRESULT hr = getValueBufferSize(pszValueName, cb);
...
HRESULT Registry::getValueBufferSize(LPCWSTR pszValueName, DWORD& dwSize) const noexcept {
    DWORD dwType;
    DWORD cbData = 0;
...
HRESULT Registry::setDWORDValue(LPCWSTR pszValueName, DWORD dwData) noexcept {
    return setValue(pszValueName, REG_DWORD, (const LPBYTE)&dwData, sizeof(dwData));
}

HRESULT Registry::setBINARYValue(LPCWSTR pszValueName, const LPBYTE pbData, int cbData) noexcept {
    return setValue(pszValueName, REG_BINARY, pbData, cbData);
}

HRESULT Registry::setSZValue(LPCWSTR pszValueName, const std::wstring& strData) noexcept {
    return setValue(pszValueName, REG_SZ, (const LPBYTE)strData.c_str(),
                    ((DWORD)strData.length() + 1) * sizeof(WCHAR));
}

HRESULT Registry::setExpandSZValue(LPCWSTR pszValueName, const std::wstring& strData) noexcept {
    return setValue(pszValueName, REG_EXPAND_SZ, (const LPBYTE)strData.c_str(),
                    ((DWORD)strData.length() + 1) * sizeof(WCHAR));
}

HRESULT Registry::setMultiSZValue(LPCWSTR pszValueName, const std::vector<std::wstring>& vStrValues) noexcept {
...
HRESULT Registry::getSubKeys(std::vector<std::wstring>& subKeys) noexcept {
...
HRESULT Registry::getValue(LPCWSTR pszValueName, DWORD dwTypeExpected, LPBYTE pbData, DWORD cbData) const noexcept {
    DWORD dwType = 0;
...
HRESULT Registry::setValue(LPCWSTR pszValueName, DWORD dwValueType, const LPBYTE pbData, DWORD cbData) noexcept {
...
bool Registry::regDeleteKey32_64(HKEY hKey, LPCWSTR pszSubKey, bool bPrefer64View) noexcept {
...
bool Registry::regDeleteSubKeys(HKEY hKey, bool bPrefer64View) noexcept {
...
BOOL Registry::regDelSubKeysRecurse(HKEY hKeyRoot, LPTSTR lpSubKey, bool bPrefer64View) noexcept {
    LPTSTR lpEnd = NULL;
    LONG lResult;
    DWORD dwSize = 0;
