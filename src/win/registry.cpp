...
namespace ashe {
namespace win {
Registry::Registry(HKEY hkeyRoot, const std::wstring& subKey) noexcept :
    hkeyRoot_(hkeyRoot),
    hkey_(NULL),
    hChangeEvent_(NULL),
    hNotifyThread_(NULL),
...
LSTATUS Registry::open(REGSAM samDesired, bool bCreate) noexcept {
    LSTATUS dwResult;
    close();
...
HKEY Registry::getHandle() const noexcept {
    return hkey_;
}
...
bool Registry::watchForChange(DWORD dwChangeFilter, bool bWatchSubtree) noexcept {
...
LSTATUS Registry::waitForChange(DWORD dwChangeFilter, bool bWatchSubtree) noexcept {
    LSTATUS ls = RegNotifyChangeKeyValue(hkey_, bWatchSubtree, dwChangeFilter, NULL, FALSE);
    return ls;
}

LSTATUS Registry::DeleteKey(HKEY hKey, LPCWSTR pszSubKey, LPCWSTR pszValName, bool bPrefer64View) noexcept {
    HKEY hSubKey = NULL;
    LSTATUS status;
...
LSTATUS Registry::DeleteSubKeys(HKEY hKeyRoot, LPCWSTR lpSubKey, bool bPrefer64View) noexcept {
...
LSTATUS Registry::getDWORDValue(LPCWSTR pszValueName, OUT DWORD& pdwDataOut) const noexcept {
    return getValue(pszValueName, REG_DWORD, (LPBYTE)(&pdwDataOut), sizeof(DWORD));
}

LSTATUS Registry::getBINARYValue(LPCWSTR pszValueName, LPBYTE pbDataOut, int cbDataOut) const noexcept {
    return getValue(pszValueName, REG_BINARY, pbDataOut, cbDataOut);
}

LSTATUS Registry::getSZValue(LPCWSTR pszValueName, OUT std::wstring& strValue) const noexcept {
    DWORD cb = 0;
...
LSTATUS Registry::getExpandSZValue(LPCWSTR pszValueName,
...
LSTATUS Registry::getMultiSZValue(LPCWSTR pszValueName, OUT std::vector<std::wstring>& vStrValues) const noexcept {
    DWORD cb = 0;
    LSTATUS ls = getValueBufferSize(pszValueName, cb);
...
LSTATUS Registry::getValueBufferSize(LPCWSTR pszValueName, DWORD& dwSize) const noexcept {
    DWORD dwType;
    DWORD cbData = 0;
...
LSTATUS Registry::setDWORDValue(LPCWSTR pszValueName, DWORD dwData) noexcept {
    return setValue(pszValueName, REG_DWORD, (const LPBYTE)&dwData, sizeof(dwData));
}

LSTATUS Registry::setBINARYValue(LPCWSTR pszValueName, const LPBYTE pbData, int cbData) noexcept {
    return setValue(pszValueName, REG_BINARY, pbData, cbData);
}

LSTATUS Registry::setSZValue(LPCWSTR pszValueName, const std::wstring& strData) noexcept {
    return setValue(pszValueName, REG_SZ, (const LPBYTE)strData.c_str(),
                    ((DWORD)strData.length() + 1) * sizeof(WCHAR));
}

LSTATUS Registry::setExpandSZValue(LPCWSTR pszValueName, const std::wstring& strData) noexcept {
    return setValue(pszValueName, REG_EXPAND_SZ, (const LPBYTE)strData.c_str(),
                    ((DWORD)strData.length() + 1) * sizeof(WCHAR));
}

LSTATUS Registry::setMultiSZValue(LPCWSTR pszValueName, const std::vector<std::wstring>& vStrValues) noexcept {
...
LSTATUS Registry::getSubKeys(std::vector<std::wstring>& subKeys) noexcept {
...
void Registry::OnChange(HKEY hkey) noexcept {
...
LSTATUS Registry::getValue(LPCWSTR pszValueName, DWORD dwTypeExpected, LPBYTE pbData, DWORD cbData) const noexcept {
    DWORD dwType = 0;
...
LSTATUS Registry::setValue(LPCWSTR pszValueName, DWORD dwValueType, const LPBYTE pbData, DWORD cbData) noexcept {
    LSTATUS ls = RegSetValueExW(hkey_, pszValueName, 0, dwValueType, pbData, cbData);
    return ls;
}

LPWSTR Registry::createDoubleNulTermList(const std::vector<std::wstring>& vStrValues) const noexcept {
...
unsigned int _stdcall Registry::NotifyWaitThreadProc(LPVOID pvParam) {
    Registry* pThis = (Registry*)pvParam;
...
LSTATUS Registry::regDeleteKey32_64(HKEY hKey, LPCWSTR pszSubKey, bool bPrefer64View) noexcept {
...
LSTATUS Registry::regDeleteSubKeys(HKEY hKey, bool bPrefer64View) noexcept {
...
LSTATUS Registry::regDelSubKeysRecurse(HKEY hKeyRoot, LPWSTR lpSubKey, bool bPrefer64View) noexcept {
    LPWSTR lpEnd = NULL;
    LSTATUS ls;
    DWORD dwSize = 0;
