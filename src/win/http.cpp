...
bool Http::openConnect(const std::wstring& url) {
    if (hSession_) {
...
bool Http::openRequest(const std::wstring& method) {
    if (hConnect_) {
...
bool Http::sendRequest(void* lpBuffer, size_t bufSize) {
    DWORD dwBufSize = 0;
...
bool Http::getResponseBody(std::vector<unsigned char>& data) {
    bool result = true;
    DWORD dwSize = 0;
    if (!hRequest_)
        return false;
...
bool Http::saveResponseBodyToFile(const std::wstring& filePath) {
    if (!hRequest_)
        return false;
...
bool Http::saveResponseBodyToFile(FILE* f) {
    if (!hRequest_)
        return false;
...
unsigned int Http::getResponseStatusCode() {
...
std::wstring Http::getResponseStatusText() {
    DWORD dwSize = 0;
...
bool Http::getResponseRawHeaders(std::wstring& headers) {
    DWORD dwSize = 0;
    WCHAR* lpOutBuffer = NULL;
...
bool Http::getResponseHeaders(std::unordered_map<std::wstring, std::wstring>& headerMap) {
...
bool Http::setRequestHeader(const std::wstring& name, const std::wstring& value) {
...
bool Http::setOption(int Option, int value) {
    BOOL bResults = FALSE;
    if (hRequest_) {
...
bool Http::setIgnoreCAIssue(bool bIgnore) {
    DWORD dwFlags = 0;
    if (bIgnore)
...
bool Http::setAllowRedirect(bool bAllow) {
...
unsigned long Http::getErrorCode() const {
    return dwGLE_;
}

bool Http::isOperationCancelled() const {
