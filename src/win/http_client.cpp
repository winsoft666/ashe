...
#include "ashe/win/http_client.h"
#include <assert.h>
...
bool HttpClient::request(std::shared_ptr<HttpReqDatagram> reqDatagram,
...
int HttpClient::doRequest(std::shared_ptr<HttpReqDatagram> reqDatagram,
                             HttpRspDatagram& rspDatagram,
                             bool allowRedirect,
                             int timeout) {
...
int HttpClient::checkErrorCode(unsigned long dwGLE) {
...
std::string HttpClient::proxy() const {
    return proxy_;
}

std::shared_ptr<Http> HttpClient::winHttp() {
    return winHttp_;
}
}  // namespace win
}  // namespace ashe