...
bool Process::Kill(const string_type& executedFileName, bool /*force*/) noexcept {
    if (executedFileName.empty())
        return false;
    ...
    if (snapshot) {
        ...
        if (Process32First(snapshot, &process)) {
            do {
                if (lstrcmpi(process.szExeFile, executedFileName.c_str()) == 0) {
                    HANDLE process_handle = OpenProcess(PROCESS_TERMINATE, FALSE, process.th32ProcessID);
                    if (process_handle) {
...
void Process::RecursiveKill(const string_type& dir, bool excludeSelf) noexcept {
    ...
    if (fhandle != INVALID_HANDLE_VALUE) {
        if (filedata.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
            if (((_tcscmp(filedata.cFileName, TEXT(".")) != 0) &&
                ...
                RecursiveKill(szTemp, excludeSelf);
            }
        }
        else {
            ...
            if (p) {
                if (_tcscmp(p, TEXT(".exe")) == 0) {
                    ...
                    if (teminate) {
                        int tryTimes = 0;
                        while (Process::Kill(filedata.cFileName, true) && ++tryTimes < 50)
                            ;
                    }
                }
            }
