...
bool AES128EncryptWithCBC(const std::vector<uint8_t>& key,
    ...
    try {
        ...
        uint8_t keyArr[AES_KEYLEN] = {0};
        uint8_t ivArr[AES_KEYLEN] = {0};
        memcpy(keyArr, key.data(), key.size());
        memcpy(ivArr, iv.data(), iv.size());

        AES_init_ctx_iv(&ctx, (const uint8_t*)keyArr, (const uint8_t*)ivArr);

        // No padding is provided so for CBC and ECB all buffers should be multiples of 16 bytes
        size_t bufSize = plain.size();
        if (bufSize % 16 != 0)
        ...
        if (buffer) {
            ...
            if (paddingMode == AESPaddingMode::PKCS7) {
                pkcs7_padding::pkcs7_padding_pad_buffer(buffer, plain.size(), bufSize, 16);
            }

            AES_CBC_encrypt_buffer(&ctx, buffer, plain.size());

            cipher.resize(bufSize);
            memcpy(cipher.data(), buffer, bufSize);
...
bool AES128DecryptWithCBC(const std::vector<uint8_t>& key,
    ...
    try {
        ...
        // No padding is provided so for CBC and ECB all buffers should be multiples of 16 bytes
        size_t bufSize = cipher.size();
        buffer = new uint8_t[bufSize];
        if (buffer) {
            memset(buffer, 0, bufSize);
            memcpy(buffer, cipher.data(), cipher.size());

            AES_CBC_decrypt_buffer(&ctx, buffer, cipher.size());

            size_t actualDataLength = 0;

            if (paddingMode == AESPaddingMode::PKCS7) {
                actualDataLength = pkcs7_padding::pkcs7_padding_data_length(buffer, cipher.size(), 16);
            }
            else if (paddingMode == AESPaddingMode::None) {
                actualDataLength = bufSize;
            }

            plain.resize(actualDataLength);
            memcpy(plain.data(), buffer, actualDataLength);
...
bool AES128EncryptWithCBC(const std::string& key,
    ...
    std::vector<uint8_t> vIV;

    vKey.resize(key.size());
    memcpy(vKey.data(), key.c_str(), key.size());

    vIV.resize(iv.size());
    memcpy(vIV.data(), iv.c_str(), iv.size());
