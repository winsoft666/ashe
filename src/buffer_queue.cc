...
ashe::BufferQueue::~BufferQueue() {
    clear();
}

std::string ashe::BufferQueue::getQueueName() const {
    return queue_name_;
}

bool ashe::BufferQueue::pushElementToFront(void* pData, size_t nDataSize) {
...
bool ashe::BufferQueue::pushElementToLast(void* pData, size_t nDataSize) {
    if (pData == nullptr || nDataSize == 0)
        return false;
...
size_t ashe::BufferQueue::popElementFromFront(void* pBuffer, size_t nBufferSize) {
    size_t rvalue = 0;
...
size_t ashe::BufferQueue::popElementFromLast(void* pBuffer, size_t nBufferSize) {
    std::lock_guard<std::recursive_mutex> lg(queue_mutex_);
    size_t rvalue = 0;
...
size_t ashe::BufferQueue::getDataFromFrontElement(void* pBuffer, size_t nBufferSize) {
    std::lock_guard<std::recursive_mutex> lg(queue_mutex_);
    size_t rvalue = 0;
...
size_t ashe::BufferQueue::getDataFromLastElement(void* pBuffer, size_t nBufferSize) {
    std::lock_guard<std::recursive_mutex> lg(queue_mutex_);
    size_t rvalue = 0;
...
size_t ashe::BufferQueue::getElementCount() {
    std::lock_guard<std::recursive_mutex> lg(queue_mutex_);
    return element_num_;
}

size_t ashe::BufferQueue::getTotalDataSize() {
    std::lock_guard<std::recursive_mutex> lg(queue_mutex_);
    return total_data_size_;
}

size_t ashe::BufferQueue::popDataCrossElement(void* pOutputBuffer, size_t nBytesToRead, size_t* pBufferIsThrown) {
    std::lock_guard<std::recursive_mutex> lg(queue_mutex_);
...
size_t ashe::BufferQueue::removeData(size_t nBytesToRemove) {
    std::lock_guard<std::recursive_mutex> lg(queue_mutex_);
    size_t rvalue = 1;
...
size_t ashe::BufferQueue::getFrontElementDataSize() {
    std::lock_guard<std::recursive_mutex> lg(queue_mutex_);
    return first_element_->size;
}

size_t ashe::BufferQueue::getLastElementDataSize() {
    std::lock_guard<std::recursive_mutex> lg(queue_mutex_);
    return last_element_->size;
}

size_t ashe::BufferQueue::toOneBuffer(char** ppBuf) {
    std::lock_guard<std::recursive_mutex> lg(queue_mutex_);
    if (ppBuf == nullptr)
        return 0;
...
size_t ashe::BufferQueue::toOneBufferWithNullEnding(char** ppBuf) {
    std::lock_guard<std::recursive_mutex> lg(queue_mutex_);
    if (ppBuf == nullptr)
        return 0;
