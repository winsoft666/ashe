...
#ifndef ASHE_WIN_SERVICE_BASE_HPP_
#define ASHE_WIN_SERVICE_BASE_HPP_
#pragma once
...
class ASHE_API WinServiceBase {
   ...
   private:
    ...
    static void WINAPI ServiceMain(DWORD dwArgc, LPWSTR* lpszArgv) {
        ...
        service->m_statusHandle = RegisterServiceCtrlHandlerW(service->m_name, ServiceCtrlHandler);
        if (service->m_statusHandle) {
            // Start the service.
            service->Start(dwArgc, lpszArgv);
        }
        else {
            EventLogTraceW(service->m_name,
                           EVENTLOG_ERROR_TYPE,
                           L"RegisterServiceCtrlHandlerW failed w/err 0x%08lx",
                           GetLastError());
        }
    }
...
#endif  // ! ASHE_WIN_SERVICE_BASE_H_
