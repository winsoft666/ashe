...
#undef MPARK_INHERITING_CTOR
  ...
  class variant {
    ...
    template <
        ...
        lib::enable_if_t<!variant_detail::is_in_place_index<Decayed>::value, int> = 0,
        lib::enable_if_t<!variant_detail::is_in_place_type<Decayed>::value, int> = 0,
  ...
  inline constexpr variant_alternative_t<I, variant<Ts...>> &&get(
      variant<Ts...> &&v) {
    return variant_detail::generic_get<I>(lib::move(v));
  }
  ...
  template <typename T, typename... Ts>
  inline constexpr T &&get(variant<Ts...> &&v) {
    return get<variant_detail::find_index_checked<T, Ts...>::value>(lib::move(v));
  }
  ...
  inline constexpr bool operator==(const variant<Ts...> &lhs,
    ...
    using equal_to = variant_detail::convert_to_bool<lib::equal_to>;
#ifdef MPARK_CPP14_CONSTEXPR
  ...
  inline constexpr bool operator!=(const variant<Ts...> &lhs,
                                   const variant<Ts...> &rhs) {
    using variant_detail::visitation::variant;
    using not_equal_to = variant_detail::convert_to_bool<lib::not_equal_to>;
#ifdef MPARK_CPP14_CONSTEXPR
  ...
  inline constexpr bool operator<(const variant<Ts...> &lhs,
                                  const variant<Ts...> &rhs) {
    using variant_detail::visitation::variant;
    using less = variant_detail::convert_to_bool<lib::less>;
#ifdef MPARK_CPP14_CONSTEXPR
  ...
  inline constexpr bool operator>(const variant<Ts...> &lhs,
                                  const variant<Ts...> &rhs) {
    using variant_detail::visitation::variant;
    using greater = variant_detail::convert_to_bool<lib::greater>;
#ifdef MPARK_CPP14_CONSTEXPR
    if (lhs.valueless_by_exception()) return false;
  ...
  inline constexpr bool operator<=(const variant<Ts...> &lhs,
                                   const variant<Ts...> &rhs) {
    using variant_detail::visitation::variant;
    using less_equal = variant_detail::convert_to_bool<lib::less_equal>;
#ifdef MPARK_CPP14_CONSTEXPR
    if (lhs.valueless_by_exception()) return true;
    if (rhs.valueless_by_exception()) return false;
    if (lhs.index() < rhs.index()) return true;
  ...
  inline constexpr bool operator>=(const variant<Ts...> &lhs,
                                   const variant<Ts...> &rhs) {
    using variant_detail::visitation::variant;
    using greater_equal = variant_detail::convert_to_bool<lib::greater_equal>;
#ifdef MPARK_CPP14_CONSTEXPR
    if (rhs.valueless_by_exception()) return true;
    if (lhs.valueless_by_exception()) return false;
    if (lhs.index() > rhs.index()) return true;
  ...
  inline constexpr DECLTYPE_AUTO visit(Visitor &&visitor, Vs &&... vs)
    DECLTYPE_AUTO_RETURN(
        (variant_detail::all(
             ...
             ? (void)0
             : throw_bad_variant_access()),
        variant_detail::visitation::variant::visit_value(lib::forward<Visitor>(visitor),
                                                 lib::forward<Vs>(vs)...))
#endif
...
namespace std {
  ...
  template <typename... Ts>
  struct hash<ashe::variant_detail::enabled_type<
      ashe::variant<Ts...>,
      ashe::lib::enable_if_t<ashe::lib::all<ashe::variant_detail::hash::is_enabled<
          ashe::lib::remove_const_t<Ts>>()...>::value>>> {
    using argument_type = ashe::variant<Ts...>;
    using result_type = std::size_t;

    inline result_type operator()(const argument_type &v) const {
      using ashe::variant_detail::visitation::variant;
      std::size_t result =
...
#ifdef MPARK_GENERIC_LAMBDAS
                    [](const auto &alt) {
                      using alt_type = ashe::lib::decay_t<decltype(alt)>;
                      using value_type = ashe::lib::remove_const_t<
    ...
    struct hasher {
      ...
      inline std::size_t operator()(const Alt &alt) const {
        using alt_type = ashe::lib::decay_t<Alt>;
        using value_type =
            ashe::lib::remove_const_t<typename alt_type::value_type>;
        return hash<value_type>{}(alt.value);
      }
    };
#endif
  ...
  template <>
  struct hash<ashe::monostate> {
    using argument_type = ashe::monostate;
    using result_type = std::size_t;
