...
class EnumFlags {
   ...
   public:
    ...
    EnumFlags(FwIter b, FwIter e, typename convertible<decltype(*std::declval<FwIter>())>::type = nullptr) 
        noexcept(noexcept(std::declval<EnumFlags>().insert(std::declval<FwIter>(), std::declval<FwIter>()))) :
        val_(0) { insert(b, e); }
